...
int soluz1(int** dist, int N)
{
	...
	{
		stop = 0; 
		comb_sempl(0, sol, i, N, 0, dist, &stop); 
		if(stop)
		{
...
void comb_sempl(int pos, int* sol, int k, int n, int start, int** dist, int* stop)
{
	...
	{
		sol[pos] = i; 
		comb_sempl(pos+1, sol, k, n, i+1, dist, stop); 
	}
}
...
void Soluz2(int** dist, int N, int* pop, int numStaz)
{
	...
	for(i=0; i<N; i++)
	{
		comb_sempl_bis(0, sol, i, dist, pop, &bestQ, numStaz, N, 0, bestSol); 
	}
...
void comb_sempl_bis(int pos, int* sol, int k, int** dist, int* pop, float* bestQ, int numStaz, int N, int start, int** bestSol)
{
	...
	if(pos>=k)
	{
		...
		pm(0, sol, k, dist, pop, bestQ, bestSol, sol2, numStaz, N);
		free(sol2); 
		return; 
	}
	...
	{
		sol[pos] = i; 
		comb_sempl_bis(pos+1, sol, k, dist, pop, bestQ, numStaz, N, i+1, bestSol); 
	}
}
...
void pm(int pos, int* val, int k, int** dist, int* pop, float* bestQ, int** bestSol, int* sol, int numStaz, int N)
{
	...
	if(pos>=k)
	{
		if( acc2(sol, k, numStaz, val) )
		{
			...
			{
				*bestQ = q; 
				copiaSolInBestSol(sol, bestSol, val, k); 
			}
		}
		return; 
	}
	
	for(i=0; i<numStaz[val[pos]]; i++)
	{
		q = ob2(sol, pos, N, dist, pop, val); 
		if( (acc1(sol, pos, numStaz, val)) && (q<*bestQ) )  //pruning
		{
			sol[pos] = i; 
			pm(pos+1, val, k, dist, pop, bestQ, bestSol, sol, numStaz, N); 
		}
	}
}
